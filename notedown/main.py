...
def convert(content, informat, outformat, strip_outputs=False):
    ...
    readers = {
        "notebook": nbformat,
        "markdown": MarkdownReader(precode="", magic=False, match="fenced"),
    }

    writers = {
        "notebook": nbformat,
        "markdown": MarkdownWriter(markdown_template, strip_outputs=strip_outputs),
    }
...
def command_line_parser():
    ...
    parser.add_argument(
        "input_file", help="markdown input file (default STDIN)", nargs="?", default="-"
    )
    parser.add_argument(
        "-o",
        "--output",
    ...
    parser.add_argument(
        ...
        help=("format to convert from, defaults to markdown or file extension"),
    )
    parser.add_argument(
        "--to",
        dest="outformat",
        choices=("notebook", "markdown"),
        help=(
            "format to convert to, defaults to notebook "
    ...
    parser.add_argument(
        ...
        "--execute",
        action="store_true",
        help=("run the notebook, executing the contents of each cell"),
    )
    parser.add_argument(
        "--timeout",
        default=30,
    ...
    parser.add_argument(
        ...
        default=[],
        help=(
            "additional code to place at the start of the "
    ...
    parser.add_argument(
        ...
        nargs="?",
        help=(
            "pre-process the markdown with knitr. "
    ...
    parser.add_argument(
        "--nomagic", action="store_false", dest="magic", help=("disable code magic.")
    )
    parser.add_argument(
        "--render",
    ...
    parser.add_argument(
        ...
        default="all",
        help=(
            "determine kind of code blocks that get "
            "converted into code cells. "
            "choose from 'all' (default), 'fenced', "
            "'strict' or a specific language to match on"
        ),
    )
    parser.add_argument("--examples", help=("show example usage"), action="store_true")
    parser.add_argument("--version", help=("print version number"), action="store_true")
    parser.add_argument("--debug", help=("show logging output"), action="store_true")
...
def main(args, help=""):
    ...
    writers = {
        "notebook": nbformat,
        "markdown": MarkdownWriter(template_file, strip_outputs=args.strip_outputs),
    }
