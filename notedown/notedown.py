...
class MarkdownReader(NotebookReader):
    ...
    def __init__(
        ...
        """
        if not code_regex:
            self.code_regex = r"({}|{})".format(self.fenced_regex, self.indented_regex)
    ...
    def pre_process_code_block(block):
        ...
        if "indent" in block and block["indent"]:
            ...
            block["content"] = re.sub(indent, "", block["icontent"], flags=re.MULTILINE)
            ...
            text_blocks.update(
                {
                    ...
                    for i, j in zip(inner_text_blocks_starts, inner_text_blocks_ends)
                }
            )
    ...
    def create_markdown_cell(block):
        ...
        kwargs = {"cell_type": block["type"], "source": block["content"]}
    ...
    def create_outputs(block):
        ...
        block.
        """
        return [nbbase.NotebookNode(output) for output in json.loads(block["content"])]
        ...
        for block in blocks:
            if (block["type"] == self.code) and (block["IO"] == "input"):
                ...
                raise NotImplementedError(
                    "{} is not supported as a celltype".format(block["type"])
                )
...
class MarkdownWriter(NotebookWriter):
    ...
    def __init__(
        ...
        logging.debug(("MarkdownWriter: template_file = %s" % template_file))
        logging.debug(
            ("MarkdownWriter.exporter.template_file = %s" % self.exporter.template_file)
        )
        logging.debug(
            (
                "MarkdownWriter.exporter.filters = %s"
    ...
    def data2uri(data, data_type):
        ...
        return uri.format(mime=mime_type, data=data[mime_type].replace("\n", ""))
...
class Knitr(object):
    ...
    def __init__(self):
        ...
        if "Warning" in stderr:
            message = (
                "Could not load knitr (needs manual installation).\n\n$ {cmd}\n{error}"
            ).format(cmd=" ".join(cmd), error=stderr)
            raise self.KnitrError(message)
    ...
    @staticmethod
    def _knit(
        fin, fout, opts_knit="progress=FALSE, verbose=FALSE", opts_chunk="eval=FALSE"
    ):
        ...
        rcmd = (
            ...
            "-e",
            script.format(
                input=fin, output=fout, opts_knit=opts_knit, opts_chunk=opts_chunk
            ),
        )

        p = subprocess.Popen(rcmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        stdout, stderr = p.communicate()
